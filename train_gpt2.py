...
# 1. Parse Args (Added for Shared Storage)
import argparse
parser = argparse.ArgumentParser(description="GPT-2 Training")
parser.add_argument("--shards_dir", type=str, default="edu_fineweb10B", help="Directory containing tokenized shards")
args = parser.parse_args()

train_loader = DataLoaderLite(B=B, T=T, process_rank=ddp_rank, num_processes=ddp_world_size, split="train", data_root=args.shards_dir)
val_loader = DataLoaderLite(B=B, T=T, process_rank=ddp_rank, num_processes=ddp_world_size, split="val", data_root=args.shards_dir)
